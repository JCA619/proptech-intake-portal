...
function buildRow(lead) {
  ...
  return [
    // --- core intake fields, front-loaded (A-U) ---
    lead.id || '',                          // A  Contact_ID
    lead.firstName || '',                   // B  FName
    lead.lastName || '',                    // C  LName
    lead.email || '',                       // D  Email
    lead.emailType || '',                   // E  Email_Type
    primaryPhone(lead),                     // F  Phone (preferred)
    lead.homePhone || '',                   // G  Home_Phone
    lead.workPhone || '',                   // H  Work_Phone
    lead.cellPhone || '',                   // I  Cell_Phone
    lead.address || '',                     // J  Address
    lead.dob || '',                         // K  DOB
    lead.contactMethod || '',               // L  Contact_Methods
    lead.bestTimeToCall || '',              // M  Best_Time_To_Call
    'Intake',                               // N  Category (always)
    lead.status || 'New',                   // O  Status
    lead.hasSecondary ? 'Yes' : 'No',       // P  Has_Secondary
    secName,                                // Q  Secondary_Name
    (lead.hasSecondary && sec.email) || '', // R  Secondary_Email
    secPhone,                               // S  Secondary_Phone
    lead.notes || '',                       // T  Notes
    lead.createdAt || '',                   // U  Submitted_At
    // --- CRM columns (kept blank, filled manually later) (V-Z) ---
    '',                                     // V  Company_ID
    '',                                     // W  Lifecycle_Stage
    '',                                     // X  Timeline
    '',                                     // Y  Interest_Tags
